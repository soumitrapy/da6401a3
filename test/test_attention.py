...
def test_attention_model(model_path, test_src, test_trg):
    ...
    model = Seq2SeqAttention(len(src_vocab), len(trg_vocab), emb_dim=64, hid_dim=64, pad_idx=trg_vocab.pad_idx, dropout=0.3, device=device)
    ...
    print(f"Attention Model Test Accuracy: {accuracy:.4f}")

    os.makedirs("predictions_attention", exist_ok=True)
    with open("predictions_attention/predictions.json", "w", encoding='utf-8') as f:
        json.dump(all_preds, f, ensure_ascii=False, indent=2)


if __name__ == '__main__':
    test_attention_model(
        model_path="models/best_attention_model.pth",
